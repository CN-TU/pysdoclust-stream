...
class SDOcluststream(Clustering):
    ...
    def __init__(self, k, T, qv=0.3, x=6, metric='euclidean', metric_params=None,
                 float_type=np.float64, seed=0, return_sampling=False, zeta=0.6, chi_min=8, chi_prop=0.05, e=3, freq_bins = 1, max_freq = 1, outlier_threshold=5.0):
    ...
    def _init_model(self, p):
        ...
        assert p['freq_bins'] > 0, 'e must be > 0'
        assert p['max_freq'] > 0, 'e must be > 0'
        ...
        self.model = cpp_obj(
            ...
            p['e'], p['p_outlier'], p['freq_bins'], p['max_freq'], p['outlier_threshold'], distance_function, p['seed']
        )
