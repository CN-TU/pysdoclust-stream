...
def optimize_sdc(trial, data, y, t=None, fixed_pams=None, int_pams=None, float_pams=None, eval_pams=None):
    ...
    for _ in range(n_eval):
        ...
        if metric == 'ari':
            ...
            # Assuming 'y' contains binary labels for outlier detection
            # ROC AUC requires true binary labels and predicted probabilities
...
def plot_boxplot(trials, param_name, metric='ari', n_bins=5, filepath=None):
    ...
    sns.boxplot(x='param_bin', y='score', data=df)
    plt.title(f"Box Plot of {metric} Scores by {param_name}")
    plt.xlabel(f'{param_name} Bins')
...
def plot_violinplot(trials, param_name, metric='ari', n_bins=5, filepath=None):
    ...
    plt.figure(figsize=(12, 6))
    sns.violinplot(x='param_bin', y='score', data=df)
...
def plot_bar(trials, param_name, metric='ari', n_bins=5, filepath=None):
    ...
    # Plot
    plt.figure(figsize=(12, 6))
    plt.errorbar(bin_centers, summary_stats['mean'], yerr=[summary_stats['mean'] - summary_stats['min'], summary_stats['max'] - summary_stats['mean']], fmt='o', capsize=5, capthick=2, ecolor='red')
...
def main(json_file, data_file):
    ...
    result_path = eval_pams.get('result_path', None)
