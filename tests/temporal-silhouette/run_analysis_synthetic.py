...
for idf, filename in enumerate(glob.glob(os.path.join(inpath, '*.arff'))):
    ...
    dsa = clustering.SDOcluststream(k=500, T=2000, e=7, chi_prop=0.1, outlier_handling=True, outlier_threshold=10.0)
    ...
    for alg_name, alg in algorithms:
        ...
        for i in range(0,data.shape[0],blocksize):
            ...
            if alg_name == 'CluStream':
                ...
                y[i:(i+blocksize)], _ = alg.fit_predict(chunk)
        ...
        df = df.append({'filename':filename, 'group':group, 'outliers': rm_outliers, 'idf':idf, 'algorithm':alg_name, 'AMI':AMI, 'Sil':Sil, 'CH':CH, 'DB':DB, 'iXB':iXB, 'iPS':iPS, 'irCIP':rCIP, 'TS':TS, 'time': end_time-start_time}, ignore_index=True)
        print(df.tail(1))
