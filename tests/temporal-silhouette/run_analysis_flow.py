...
res = []
# df_columns=['filename','group','outliers','idf','algorithm','AMI','Sil','CH','DB','iXB','iPS','irCIP','TS','time']
df_columns=['filename','group','outliers','idf','algorithm','AMI','time']
df = pd.DataFrame(columns=df_columns)
pd.set_option('display.float_format', '{:.10f}'.format)

outfile_csv = outpath + "results_flow.csv"
outfile_tex = outpath + "results_flow.tex"
...
for idf, filename in enumerate(glob.glob(os.path.join(inpath, '*.arff'))):
    ...
    dsa = clustering.SDOcluststream(k=2500, T=20000, e=7, chi_prop=0.02)
    grt = []
    blocksize = 2000

    algorithms = (("SDOstreamc", dsa),("CluStream", cls),("DenStream", den),("BIRCH", bir),("GT", grt))

    print("Dataset shape:", data.shape)
    print("Blocksize:", blocksize)

    df_algs=['SDOstreamc','CluStream','DenStream','BIRCH','GT']
    ...
    for alg_name, alg in algorithms:        
        ...
        # df = df.append({'filename':filename, 'group':group, 'outliers': rm_outliers, 'idf':idf, 'algorithm':alg_name, 'AMI':AMI, 'Sil':Sil, 'CH':CH, 'DB':DB}, ignore_index=True)
        df = df.append({'filename':filename, 'group':group, 'outliers': rm_outliers, 'idf':idf, 'algorithm':alg_name, 'AMI':AMI, 'time':end_time-start_time}, ignore_index=True)
