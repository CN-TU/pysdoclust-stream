...
//     typedef typename MinHeap::handle_type handle_type;
//     typedef typename MinHeap::iterator iterator;  // Iterator for kHeap heap
//     typedef typename MinHeap::ordered_iterator ordered_iterator;  // Ordered iterator for kHeap heap    
//   private:
//     MinHeap kHeap; // Max heap for k lowest elements, largest on top
...
//             kHeapMap[it->second] = boost::heap::binomial_heap< PairType, boost::heap::compare<std::greater<PairType>>>::s_handle_from_iterator(it);
//         }
//     }
...
//     bool insert(const T key, const ValueType val) {
//         if (kHeapMap.count(key)>0) {return false; }
//         handle_type ha = kHeap.push(PairType(val, key));
//         kHeapMap[key] = ha;
//         return true;
//     }
//     bool insert(const PairType pair) { return insert(pair.second, pair.first); }
//     const PairType pop() {
//         PairType pair = kHeap.top();
//         kHeap.pop();
//         kHeapMap.erase(pair.second);
//         return pair;
//     }
//     ValueType operator[](const T& key) const {        
//         if (kHeapMap.count(key)>0) { return (*(kHeapMap[key])).first; }
...
class KBufferHeap {
  ...
  private:
    ...
    std::unordered_map<T, rhandle_type> bufferMap;  
    ...
    bool empty_b() const { return rBuffer.empty(); }
    ValueType get_h(const T& key) const { return (*(kHeapMap.at(key))).first; }
    ValueType get_b(const T& key) const { return (*(bufferMap.at(key))).first; }
    ...
    bool insert_b(const T key, const ValueType val) {
        if ( check_b(key, val)) { 
            rhandle_type ha = rBuffer.push(PairType(val, key));
            bufferMap[key] = ha;
            return true;
        }        
        return false;
    ...
    bool update_b(const T oldKey, const T key, const ValueType val) {
        if ( in_b(oldKey) && check_b(key, val, false) ) {
            rhandle_type ha = bufferMap[oldKey];
            rBuffer.update(ha, PairType(val, key));
  ...
  public:
    ...
    KBufferHeap(const KBufferHeap& other) {
        ...
        for (riterator it = rBuffer.begin(); it != rBuffer.end(); ++it) {            
            bufferMap[it->second] = boost::heap::binomial_heap< PairType, boost::heap::compare<std::greater<PairType>>>::s_handle_from_iterator(it);
        }
    ...
    bool deactivate(const T& key) {
        if (active(key)) {
            PairType pair;
            if (in_h(key)) { pair = *(kHeapMap[key]); }
            if (in_b(key)) { pair = *(bufferMap[key]); } 
    ...
    bool swap_active(const T& key_a, const T& key_ia) {
        if (active(key_a) && in_ia(key_ia)) {
            ...
            PairType pair_a;
            if (in_h(key_a)) { pair_a = *(kHeapMap[key_a]); }
            if (in_b(key_a)) { pair_a = *(bufferMap[key_a]); }  
