...
#ifndef TPSDOSTREAMC_TREE_H
#define TPSDOSTREAMC_TREE_H

template<typename FloatType, template ObservationType>
void tpSDOStreamc<FloatType,ObservationType>::fit_impl(
    ...
    for (const auto& neighbor : nearestNeighbors) {
        ...
        if (idx!=observer_index) {
            if (temporary_scores.count(idx) > 0) {                
                ...
                ObservationType& observations = value_pair.first;
                ...
                ObservationType observations(freq_bins);
                for (std::size_t freq_ind = 0; freq_ind < freq_bins; freq_ind++) {
                    observations[freq_ind] = score_vector[freq_ind];
                }
...
void tpSDOStreamc<FloatType,ObservationType>::fit_impl(
        ...
        const FloatType& now,           
        const int& current_observer_cnt,
        const int& current_neighbor_cnt) override {   
...
std::vector<int> tpSDOstreamc<FloatType,ObservationType>::fitPredict_impl(
        const std::vector<Vector<FloatType>>& data, 
        const std::vector<FloatType>& time_data, 
        bool fit_only) override {
